...
def svm_loss_vectorized(W, X, y, reg):
  ...
  #############################################################################
  new_margin = margin 
  new_margin[new_margin > 0.0] = 1.0   #new_margin = m*class
  #list = np.sum(new_margin, axis = 1)  # list = m
  dW = X.T.dot(new_margin)
  list = np.sum(new_margin, axis = 1)
  temp = np.zeros(margin.shape)
  temp[range(num_train), y] = list
  dW -= X.T.dot(temp)
  dW /= num_train
  dW += 2 * reg * W
  pass
