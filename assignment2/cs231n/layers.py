...
def affine_backward(dout, cache):
    ...
    ###########################################################################
    dx = dout.dot(w.T).reshape(x.shape) # N*M dot 
    dw = x.reshape((x.shape[0], -1)).T.dot(dout) # N*D dot N*M
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        #samp_mean =
        cache = {}
        sample_var = np.var(x, axis = 0) #shape is (1, D)
        sample_mean = np.mean(x, axis = 0) #shape is (1, D)
        x_hat = (x - sample_mean.reshape((1, x.shape[1]))) / np.sqrt(sample_var.reshape((1, x.shape[1])) + eps)
        running_mean = momentum * running_mean + (1 - momentum) * sample_mean
        running_var = momentum * running_var + (1 - momentum) * sample_var
...
def batchnorm_backward(dout, cache):
    ...
    dsample_mean = np.sum(-1.0 / np.sqrt(sample_var+eps)* dx_hat, axis = 0) + 1.0/N*dsample_var *np.sum(-2*(x-sample_mean), axis = 0) 
    dx = dx_hat * 1.0 / np.sqrt(sample_var + eps)  + 2.0 * dsample_var / N * (x - sample_mean) + dsample_mean / N
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    ###########################################################################
    N, C, H, W = x.shape
    x_change = x.transpose(1, 0, 2, 3).reshape(C, N*H*W).T
    out, cache = batchnorm_forward(x_change, gamma, beta, bn_param)
...
def spatial_batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    N, C, H, W = dout.shape
    dout_change = dout.transpose(1, 0, 2, 3).reshape(C, N*H*W).T
    dx, dgamma, dbeta = batchnorm_backward(dout_change, cache)
